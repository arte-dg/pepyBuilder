...
final class Plugin {
	...
	private function __construct() {
		...
		add_action( 'jupiterx_init', [ $this, 'check_theme_version' ], 10 );
	...
	public function check_elementor_version() {
		...
		if ( ! Utils::is_plugin_installed( 'elementor' ) ) {
		...
		if ( ! did_action( 'elementor/loaded' ) ) {
			if ( current_user_can( 'activate_plugins' ) ) {
				add_action( 'admin_notices', [ $this, 'admin_notice_inactive_elementor_plugin' ] );
			}
			// don't go further.
			return;
		}
	...
	/**
	 * Check Jupiter X theme version requirement.
	 *
	 * @return void
	 */
	public function check_theme_version() {
		add_action( 'admin_notices', function() {
			if ( version_compare( JUPITERX_VERSION, '1.3.0', '<' ) && ( current_user_can( 'manage_network_themes' ) || current_user_can( 'update_themes' ) ) ) {
				?>
				<div class="notice notice-error">
					<p>
						<?php esc_html_e( 'Raven - This plugin requires Jupiter X theme v1.3.0 and above. Please update your theme to the latest version.', 'raven' ); ?>
					</p>
				</div>
				<?php
			}
